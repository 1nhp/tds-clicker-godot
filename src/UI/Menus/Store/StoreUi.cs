...
public partial class StoreUi : MenuBase
{
	...
	private void _PlayUiAnim(UiAnimTypes type)
	{
		switch (type)
		{
	...
	private async void _MenuOpened()
	{
		...
		_PlayUiAnim(UiAnimTypes.Header);
		await ToSignal(GetTree().CreateTimer(0.2f), "timeout");
		_PlayUiAnim(UiAnimTypes.TabButtons);
	}
	
	private void SwitchTab(Panel tab)
	{
		_PlayUiAnim(UiAnimTypes.Default);
	...
	private void _onStoreManagerLoadingFinished()
	{
		...
		{
			if (resource is StoreData item)
				switch (item.Type)
				{
					case "Enemy":
						CreateButton(item, EnemyContainer);
						break;
					case "Drooper":
						CreateButton(item, DrooperContainer);
						break;
				}
		}
	}
	...
	private void _UpdateContent(StoreData item)
	{
		switch (item.Type)
		{
			case "Enemy": 
	...
	private void _onItemClicked(Button button)
	{
		...
		_UpdateContent(StoreManager.Instance.CurrentItem);
		_PlayUiAnim(UiAnimTypes.Content);
	}
	...
	{
		GameManager.Instance.UpdateCounter(GameManager.CounterTypes.Droopers);
		GameManager.Instance.UpdateCounter(GameManager.CounterTypes.Income);
